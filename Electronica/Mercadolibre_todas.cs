...
{
	public class Mercadolibre_todas : Form
	{
		...
		{
			string query_tabla_clientes = "SELECT * FROM `refacciones_tv` WHERE CONCAT(`id_refacciones`, `pieza`, `marcas`, `modelos`, `cantidad`,`almacen`,`estado`, `precio`,'fecha_entrada','fecha_salida','etiqueta_1','etiqueta_2')LIKE '%" + valueToSearch + "%'";
		...
		private void TablaClientes_CellMouseClick(object sender, DataGridViewCellMouseEventArgs e)
		{
			if (e.RowIndex >= 0)
			{
				...
				Mercadolibre_vista cl = new Mercadolibre_vista();
				...
				cl.txtalmacen.Text = row.Cells["almacen"].Value.ToString();
