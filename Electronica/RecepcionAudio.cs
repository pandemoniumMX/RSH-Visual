...
{
	public class RecepcionAudio : Form
	{
		...
		private void gtngenorden_Click(object sender, EventArgs e)
		{
			...
			else
			{
				...
				string query_insertar_cel = "insert into reparar_audio (equipo, marca, modelo, accesorios, falla, comentarios,fecha_entregar,fecha_egreso, servicio, estado,ubicacion, id_folio) values ('" + equipo + "', '" + marca + "', '" + modelo + "','" + accesorios + "', '" + falla + "', '" + comentarios + "', '" + fecha + "','" + fecha + "' ,'" + locacion + "','Pendiente','Recepcion', '" + idfolio + "') ";
		...
		{
			if (string.IsNullOrEmpty(combolocacion.Text))
			{
				MessageBox.Show("Debe completar la informacion");
			}
		}
		...
		private void InitializeComponent()
		{
			...
			label1.TabIndex = 0;
			label1.Text = "Orden de Servicio Audio y Sonido";
			...
			gtngenorden.Location = new System.Drawing.Point(659, 267);
