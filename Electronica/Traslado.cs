...
namespace Electronica
{
	public class Traslado : Form
	{
		...
		{
			Traslado_nuevo ss = new Traslado_nuevo();
			ss.ShowDialog();
		}
		...
		private void TablaClientes_CellMouseClick(object sender, DataGridViewCellMouseEventArgs e)
		{
			if (e.RowIndex >= 0)
			{
				...
				Traslado_actualizar cl = new Traslado_actualizar();
				cl.txtidtraslado.Text = row.Cells["id_traslado"].Value.ToString();
				...
				cl.comboubicacion.Text = row.Cells["ubicacion"].Value.ToString();
				cl.combodestino.Text = row.Cells["destino"].Value.ToString();
				cl.txtfecha.Text = row.Cells["fecha_solicitud"].Value.ToString();
		...
		private void InitializeComponent()
		{
			...
			label2.TabIndex = 2;
			label2.Text = "Traslados pendientes";
			...
			btn_cliente_nuevo.Text = "    Nueva Solicitud";
